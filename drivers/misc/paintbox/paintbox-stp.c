		...
		uint8_t sram_target, size_t len_bytes)
{
	...
	case STP_RAM_TARGET_SCALAR_RAM:
		...
		break;
	default:
		dev_err(&pb->pdev->dev, "%s: stp%u: invalid ram transfer type"
				"\n", __func__, stp->stp_id);
		return -EINVAL;
	};
	...
	if (sram_addr_bytes + len_bytes > sram_len_bytes) {
		dev_err(&pb->pdev->dev, "%s: stp%u: memory transfer out"
					" of range %lu > %lu bytes\n", __func__,
					stp->stp_id, len_bytes, sram_len_bytes);
		return -ERANGE;
	}
