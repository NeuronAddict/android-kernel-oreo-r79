		...
		size_t len)
{
	...
	ret = paintbox_dump_stp_reg_verbose(pb, STP_CTRL, val, buf, &written,
			len,
	...
	ret = paintbox_dump_stp_reg_verbose(pb, STP_STAT, val, buf, &written,
			len,
...
void paintbox_stp_debug_init(struct paintbox_data *pb, struct paintbox_stp *stp)
{
	paintbox_debug_create_entry(pb, &stp->debug, pb->debug_root, "stp",
			stp->stp_id, paintbox_dump_stp_registers,
