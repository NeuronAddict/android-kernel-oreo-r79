...
{
	dev_info(&pb->pdev->dev, "lbp%u lb%u setup\n", config->lb_pool_id,
			config->lb_id);
	dev_info(&pb->pdev->dev, "\twidth %upx height %upx channels %u\n",
			config->width_pixels, config->height_pixels,
			config->num_channels);
	dev_info(&pb->pdev->dev,
			"\tfb offset %upx X offset %dpx Y offset %dpx chan "
			"offset %dpx\n", config->fb_offset_pixels,
			config->x_offset_pixels, config->y_offset_pixels,
			config->chan_offset_pixels);
	dev_info(&pb->pdev->dev,
			"\trptrs %u reuse rows %u fb rows %u sb rows %u\n",
			config->num_read_ptrs, config->num_reuse_rows,
			config->fb_rows, config->sb_rows);
	dev_info(&pb->pdev->dev, "\tfb base addr 0x%08x sb base addr 0x%08x\n",
			config->ipu_fb_base_addr, config->ipu_sb_base_addr);
	dev_info(&pb->pdev->dev, "\tpadding method %u value %u\n",
...
static inline int paintbox_dump_lbp_reg(struct paintbox_data *pb,
...
static int paintbox_dump_lbp_reg_verbose(struct paintbox_data *pb,
		uint32_t reg_offset, char *buf, int *written, size_t len,
		const char *format, ...)
{
	va_list args;
	...
	ret = paintbox_dump_lbp_reg(pb, reg_offset, buf, written, len);
	if (ret < 0)
		return ret;
		...
		size_t len)
{
	...
	ret = paintbox_dump_lbp_reg_verbose(pb, LBP_SEL, buf, &written, len,
	...
	ret = paintbox_dump_lbp_reg_verbose(pb, LBP_CTRL, buf, &written, len,
	...
	ret = paintbox_dump_lbp_reg(pb, LBP_STAT, buf, &written, len);
	if (ret < 0)
		return ret;

	val = readq(pb->lbp.reg_base + LBP_CAP0);
	ret = paintbox_dump_lbp_reg_verbose(pb, LBP_CAP0, buf, &written, len,
	...
	ret = paintbox_dump_lbp_reg_verbose(pb, LBP_CAP1, buf, &written, len,
			"\tMEM_SIZE %u bytes\n", val);
	if (ret < 0)
		return ret;
		...
		size_t len)
{
	...
	ret = paintbox_dump_lbp_reg_verbose(pb, LB_CTRL0, buf, &written, len,
	...
	ret = paintbox_dump_lbp_reg_verbose(pb, LB_OFFSET, buf, &written, len,
	...
	ret = paintbox_dump_lbp_reg_verbose(pb, LB_BDRY, buf, &written, len,
	...
	ret = paintbox_dump_lbp_reg_verbose(pb, LB_IMG_SIZE, buf, &written, len,
	...
	ret = paintbox_dump_lbp_reg_verbose(pb, LB_SB_SIZE, buf, &written, len,
			"\tSB_ROWS %u SB_COLS %u\n",
	...
	ret = paintbox_dump_lbp_reg_verbose(pb, LB_BASE, buf, &written, len,
	...
	ret = paintbox_dump_lbp_reg_verbose(pb, LB_STAT, buf, &written, len,
	...
	ret = paintbox_dump_lbp_reg_verbose(pb, LB_L_PARAM, buf, &written, len,
			"\tL_WIDTH %u L_INC %u\n",
		...
		const char *msg)
{
	...
	paintbox_dump_lbp_registers(&pb->lbp.lbps[lbp->pool_id].debug,
			pb->vdbg_log, pb->vdbg_log_len);
	dev_info(&pb->pdev->dev, "%s\nlbp%u:\n%s", msg, lbp->pool_id,
			pb->vdbg_log);

	paintbox_dump_lb_registers(
			&pb->lbp.lbps[lbp->pool_id].lbs[lb->lb_id].debug,
			pb->vdbg_log, pb->vdbg_log_len);
